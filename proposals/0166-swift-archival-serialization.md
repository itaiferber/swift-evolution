...
public protocol KeyedEncodingContainerProtocol {
    ...
    ///
    /// - parameter key: The key to associate the null value with.
    ...
    /// Encodes the given object conditionally for the given key.
    ///
    ...
    ///
    /// A default implementation for these is given in terms of the corresponding `encode(_:forKey:)` method.
    ///
    /// - parameter value: The value to encode.
    /// - parameter key: The key to associate the value with.
    /// - throws: `EncodingError.invalidValue` if the given value is invalid in the current context for this format.
...
public struct KeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    // + KeyedEnodingContainerProtocol requirements
}
...
public protocol KeyedDecodingContainerProtocol {
    ...
    /// - throws: `DecodingError.keyNotFound` if `self` does not have an entry for the given key.
    /// - throws: `DecodingError.valueNotFound` if `self` if the encountered encoded value is null.
    ...
    ///
    /// A default implementation of these methods is given in terms of the corresponding `decode(_:forKey:)` and `decodeNil(forKey:)` calls.
    ///
    /// - parameter type: The type of value to decode.
    /// - parameter key: The key that the decoded value is associated with.
...
public struct KeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
    ...
    // + KeyedDecodingContainerProtocol requirements
}
```
...
public protocol UnkeyedEncodingContainer {
    ...
    /// Encodes the given object conditionally.
    ///
...
public protocol UnkeyedDecodingContainer {
    ...
    ///
    /// A default implementation of these methods is given in terms of the corresponding `decode(_:)` and `decodeNil()` calls.
    ///
    /// - parameter type: The type of value to decode.
...
#### Localization & `NSError` Interoperability

In order to make the above errors presentable to end users, we will add the following extensions to the standard library to allow for native bridging between these types and `NSError`:
...
When Foundation is loaded, the above extensions allow for `EncodingError` and `DecodingError` to bridge natively to `NSError` instances, complete with localized descriptions provided by Foundation (based on the error code):
...
The `Error` → `NSError` code mapping we provide will be as follows:

* `DecodingError.keyNotFound`, `DecodingError.valueNotFound` → `NSCoderValueNotFoundError`
* `DecodingError.typeMismatch`, `DecodingError.dataCorrupted` → `NSCoderReadCorruptError`
* `EncodingError.invalidValue` → `NSCoderInvalidValue`
 
These errors will also expose the `EncodingError`/`DecodingError`'s `Context.debugDescription` and `.underlyingError` in their user info dictionaries under the `NSDebugDescriptionErrorKey` and `NSUnderlyingErrorKey`, respectively, and we will provide a new `NSCodingPathErrorKey` to access the `Context.codingPath` as well.

#### `.dataCorrupted` Conveniences

Of these new errors, `EncodingError.invalidValue`, `DecodingError.typeMismatch`, `.valueNotFound`, and `.keyNotFound` will rarely, if ever, be thrown from API consumer code.

However, `DecodingError.dataCorrupted` _is_ an error that will often be thrown from user code, to indicate invalid data found during an `init(from:)`. Throwing such an error can be quite verbose, so the following conveniences are offered to throw such an error:
...
Usage of these conveniences simplifies error throwing:

```swift
// Before (in a keyed container):
...
// Now:
throw DecodingError.dataCorruptedError(forKey: .myKey, in: container, debugDescription: "Something went wrong.")
```
...
public protocol KeyedEncodingContainerProtocol {
    ...
    /// Encodes the given value for the given key if it is not `nil`.
    ///
    /// A default implementation of these methods is given in terms of the corresponding `encode(_:forKey:)` call.
    ///
    /// - parameter value: The value to encode.
    /// - parameter key: The key to associate the value with.
    /// - throws: `EncodingError.invalidValue` if the given value is invalid in the current context for this format.
...
public struct KeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    // + KeyedEncodingContainerProtocol requirements
}
